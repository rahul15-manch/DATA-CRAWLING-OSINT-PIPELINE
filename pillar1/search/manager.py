...
class SearchManager:
    ...
    def __init__(self) -> None:
        ...
        self.provider_budgets = {
            "google_html": 8,
            "playwright_google": 10,
            "linkedin_playwright": 10,
            "duckduckgo": 4,
            "bing": 2
        }
    ...
    def search(
        ...
        if target_leads_limit > 0 and getattr(self, "discovered_leads_count", 0) >= target_leads_limit:
            print(f"[SearchManager] TARGET_LEADS_LIMIT ({target_leads_limit}) reached ({self.discovered_leads_count} leads). Skipping query: '{query}'")
            return []
        ...
        import inspect
        ordered_func = self._get_ordered_providers
        sig = inspect.signature(ordered_func)
        has_args = False
        for param in sig.parameters.values():
            if param.kind in (inspect.Parameter.POSITIONAL_OR_KEYWORD, inspect.Parameter.POSITIONAL_ONLY, inspect.Parameter.VAR_POSITIONAL):
                has_args = True
                break
        if has_args:
            ordered = ordered_func(query)
        else:
            try:
                ordered = ordered_func()
            except TypeError:
                ordered = ordered_func(query)
        ...
        for provider in ordered:
            ...
            # Cooldown / consecutive blocks check for Google / Playwright Google
            if pname in ("google_html", "playwright_google"):
                if self._consecutive_blocks[pname] >= 3:
                    disabled_until = self._provider_disabled_until.get(pname, 0.0)
                    if pname == "google_html" and self._google_disabled_until > disabled_until:
                        disabled_until = self._google_disabled_until
                    if time.time() < disabled_until:
                        remaining_cooldown = int(disabled_until - time.time())
                        print(f"[SearchManager] Skipping {pname} (Disabled globally for {remaining_cooldown}s due to 3 consecutive blocks)")
                        continue
                    else:
            ...
            if pname in ("google_html", "playwright_google"):
                ...
                if zeroes >= 3:
                    ...
                    overall_yield = total_accepted / total_results
                    if overall_yield < 0.10:
                        print(f"[SearchManager] Skipping {pname} for family {family} (Adaptive Stop: {zeroes} zeroes, yield {overall_yield:.2%})")
                        continue
            else:
    ...
    def _is_provider_eligible(self, provider_name: str, query: str) -> bool:
        """Determines if a provider is eligible to run the given query."""
        if provider_name == "linkedin_playwright":
            query_lower = query.lower()
            return "linkedin.com" in query_lower or "linkedin" in query_lower
        return True

    def _get_ordered_providers(self, query: str = "") -> list[SearchProvider]:
        """
        ...
        - Filters out providers not eligible for the given query
        """
        ...
        for name in self._priority:
            if not ProviderRegistry.get_provider_class(name):
                ...
                continue
            if query and not self._is_provider_eligible(name, query):
                continue
            instance = self._get_instance(name)
