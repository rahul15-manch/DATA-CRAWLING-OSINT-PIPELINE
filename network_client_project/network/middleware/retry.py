...
class RetryMiddleware(BaseMiddleware):
    ...
    def process_response(self, request: Request, response: Response, client: Any) -> Union[Request, Response]:
        ...
        if is_search:
            waf_error = ErrorDetector.detect_waf_or_captcha(response)
            if response.status_code in (403, 429) or waf_error:
                if waf_error and response.status_code == 200:
                    response.status_code = 403
                logger.warning(f"[Retry] Search provider {request.meta.get('provider')} hit block/CAPTCHA/429 (HTTP {response.status_code}).")
                retry_count = request.meta.get("retry_count", 0)
                if retry_count < 1 and request.meta.get("_proxy_obj"):
                    proxy_obj = request.meta.get("_proxy_obj")
                    if proxy_obj:
                        request.meta["exclude_urls"] = request.meta.get("exclude_urls", set()) | {proxy_obj.raw_url}
                    logger.info(f"[Retry] Rotating proxy for search provider {request.meta.get('provider')}")
                    return self._retry(request, response, reason=f"Search Provider Block ({waf_error or response.status_code})", status_code=response.status_code, keep_proxy=False)
                return response
    ...
    def _retry(
        ...
        deadline = request.meta.get("deadline")
        if deadline and deadline.is_exceeded():
            logger.warning(f"[Retry] Deadline exceeded ({deadline.remaining():.1f}s remaining). Aborting retry loop for {request.url}")
        ...
        if retry_times < max_retries:
            ...
            if deadline:
                ...
                if rem <= 1.0:
                    logger.warning(f"[Retry] Insufficient time for retry delay ({rem:.1f}s remaining). Aborting retry.")
                    return response if response else None
                backoff = min(backoff, max(0.0, rem - 0.5))
