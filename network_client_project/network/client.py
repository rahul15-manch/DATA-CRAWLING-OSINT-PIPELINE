        ...
        while True:
            ...
            if deadline:
                if deadline.is_exceeded():
                    ...
                    logger.warning(f"[NetworkClient] Deadline exceeded ({deadline.remaining():.1f}s remaining). Refusing request to {request.url}")
                    raise DeadlineExceeded(f"NetworkClient deadline exceeded for {request.url}")
            ...
            try:
                ...
                eff_timeout = request.timeout or max(config.CONNECT_TIMEOUT, config.READ_TIMEOUT)
                if deadline:
                    rem = deadline.remaining()
                    if rem > 0:
                        eff_timeout = min(eff_timeout, rem)
