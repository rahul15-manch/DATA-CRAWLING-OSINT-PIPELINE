...
class ProxyManager:
    def __init__(self):
        ...
        # Cooldown: prevent re-validation within 600s of the last pass.
        self._last_validation_time: float = 0.0   # time.monotonic()
        # Concurrency guard: ensure only one validation thread runs at a time.
    ...
    def validate_all_proxies_async(self):
        ...
        Protected by a 600-second cooldown and a concurrency lock so that repeated
        load_from_list / load_from_file calls never spawn overlapping validation passes.
        """
        _VALIDATION_COOLDOWN_S = 600.0

        # Fast path: check cooldown without acquiring the validation lock
        now = time.monotonic()
        if now - self._last_validation_time < _VALIDATION_COOLDOWN_S:
            remaining_cooldown = int(_VALIDATION_COOLDOWN_S - (now - self._last_validation_time))
            logger.debug(
                f"[ProxyManager] Skipping background validation — last run was "
                f"{int(now - self._last_validation_time)}s ago (cooldown {remaining_cooldown}s remaining)."
            )
            return

        def _check_all():
            # Double-checked locking: re-check inside the lock to handle races
            with self._validation_lock:
                now2 = time.monotonic()
                if now2 - self._last_validation_time < _VALIDATION_COOLDOWN_S:
                    logger.debug("[ProxyManager] Validation already ran recently (race resolved). Skipping.")
                    return
                # Mark the start time immediately to block concurrent launchers
                self._last_validation_time = now2
            ...
            def _test_single_proxy(p: Proxy):
                ...
                for test_url in GENERAL_TEST_URLS:
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            if "safeview-static" in resp_text or "menlo.gutsenv.net" in resp_text or "<title>secureweb</title>" in resp_text:
                                continue  # Menlo intercept is not genuine internet connectivity
                            general_alive = True
                            break
                    except Exception:
                ...
                try:
                    ...
                    resp_text = (getattr(resp, "text", "") or "").lower()
                    if resp.status_code == 200 and not ("safeview-static" in resp_text or "menlo.gutsenv.net" in resp_text or "<title>secureweb</title>" in resp_text):
                        ...
                        logger.warning(f"[ProxyManager] Proxy {p.raw_url} -> Google HTTP {resp.status_code} (blocked or Menlo intercept)")
