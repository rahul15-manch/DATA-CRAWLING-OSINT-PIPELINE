...
def run(keyword: str, disable_cache: bool = False) -> None:
    ...
    pipeline_result = pillar1_run(keyword)

    if isinstance(pipeline_result, PipelineResult):
        raw_file_str = pipeline_result.raw_file
        status = pipeline_result.status
        count = pipeline_result.record_count
    elif isinstance(pipeline_result, str):
        raw_file_str = pipeline_result
        status = "success"
        count = 0
    else:
        raw_file_str = getattr(pipeline_result, "raw_file", None)
        status = getattr(pipeline_result, "status", "unknown")
        count = getattr(pipeline_result, "record_count", 0)

    if not raw_file_str:
        print(f"  Pillar 1 returned no output file (status: {status!r}). Exiting.")
        sys.exit(1)

    raw_file = pathlib.Path(raw_file_str)
    _require_file(raw_file, "clean_leads.py")
    _ok(f"Raw lead cards created ({count} leads)", raw_file)
